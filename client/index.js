	...
	bot.on('text', function (msge) {
		...
		console.log('this is message: ' + messages.text);
				 ...
				 bot.once("text",(mess)=>{
                    ...
                    bot.sendMessage(messages.chat.id, " Назва проблеми = > " + mess.text.toUpperCase() + " вкажіть її вирішення  \ud83d\udd50  ")
					   ...
					   .then(()=>{
										...
										if (result.data.length<=0) {
											bot.sendMessage(messages.chat.id, '``'+ '*'+ "нажаль за Вашим запитом :  = > " + mess.text.toUpperCase() + " " + " нічого не знайдено   повторіть спробу" + '\n'+ 
						...
						//
						    bot.once("text",(messa)=>{
					            console.log("this is messa.text " + " " + messa.text + ' '  +  mess.text.length);
                                    ...
                                    bot.sendMessage(messages.chat.id, " вирішення проблеми :  = > " + messa.text.toUpperCase() + " отже :  \ud83d\udd50  ")
									  .then(()=>{
